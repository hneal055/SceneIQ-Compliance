...
EXPENSES_BY_PRODUCTION = {
    0: [  # First production — feature film, large budget
        ...
        ("other", "Music rights — score + clearances", 35000, False),
    ],
    1: [  # Second production — TV series
        ...
        ("labor", "Production Coordinator", 38000, True),
    ...
    2: [  # Third production — documentary
        ...
        ("post_production", "Color + sound mix", 22000, True),
        ("post_production", "Music licensing", 18000, False),
        ("legal", "E&O + legal", 12000, False),
    ],
    3: [  # Fourth production — horror feature
        ("labor", "Director", 45000, True),
        ("labor", "DP — genre specialist", 38000, True),
        ("labor", "Production Designer — practical FX", 32000, True),
        ("labor", "Makeup / SFX team", 65000, True),
        ("labor", "1st AD", 22000, True),
        ("equipment", "Camera package — RED V-Raptor", 28000, True),
        ("equipment", "Lighting — low-light package", 18000, True),
        ...
        ("post_production", "Editorial + online", 45000, True),
        ("post_production", "Sound design + mix", 32000, True),
        ("post_production", "Score — original music", 28000, True),
...
MONITORING_EVENTS = [
    {
        ...
        "summary": "The Georgia Film Office has issued updated guidance on resident payroll verification requirements. Productions must now provide county-level residency confirmation in addition to state tax filings. Effective for productions beginning principal photography after March 1, 2026.",
        "url": "https://www.georgia.org/film",
        "sourceId": None,
    },
    {
        "title": "California Film Commission — Round 4 Application Period Opens",
        "summary": "The California Film Commission has opened Round 4 applications for the Film & Television Tax Credit Program 3.0. Total allocation is $330M. Applications accepted through May 15, 2026. Competitive scoring based on jobs created, diversity plans, and California spend ratio.",
        "url": "https://film.ca.gov/tax-credit/",
        "sourceId": None,
    },
    {
        "title": "New Mexico — Streaming Series Uplift Extended Through 2028",
        "summary": "New Mexico has extended its 5% additional uplift for streaming series productions through December 31, 2028. Productions qualifying under the streaming series designation may now claim up to 35% on qualified New Mexico expenditures.",
        "url": "https://nmfilm.com/incentives/",
        "sourceId": None,
    },
    {
        "title": "UK Film Tax Relief — BFI Cultural Test Guidance Updated",
        "summary": "The BFI has published updated guidance for the Cultural Test under the UK Film Tax Relief. Changes affect scoring in the Cultural Content and Cultural Contribution categories. Productions in development should review the updated test criteria before submitting.",
        "url": "https://www.bfi.org.uk/get-funding-and-support/film-tax-relief",
        "sourceId": None,
    },
    {
        "title": "Louisiana — Transferable Credit Market Rate Update",
        "summary": "Current market rates for Louisiana transferable entertainment tax credits are trading between 86-89 cents on the dollar. Demand remains strong from corporate tax buyers. Productions with certified credits are advised to engage brokers early in the process.",
...
async def seed_expenses(production, expense_data):
    existing = await prisma.expense.count(
    ...
    for i, (category, description, amount, qualifying) in enumerate(expense_data):
        ...
        await prisma.expense.create(data={
            ...
            "vendorName": f"{description.split('—')[0].strip()} LLC",
        })
        count += 1
...
async def main():
    print("\n" + "=" * 60)
    print("SceneIQ Demo Pre-Load Script")
    print("=" * 60)
    ...
    for i, production in enumerate(productions):
        ...
        total_expenses += await seed_expenses(production, expense_data)
