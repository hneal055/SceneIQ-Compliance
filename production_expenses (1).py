...
@router.patch("/productions/{production_id}/expenses/{expense_id}",
              summary="Update an expense (inline edit)")
async def update_expense(production_id: str, expense_id: str, data: ExpenseUpdate):
    ...
    if data.isQualifying is not None: update_data["isQualifying"] = data.isQualifying
    if data.vendorName  is not None: update_data["vendorName"]  = data.vendorName
    ...
    if data.expenseDate is not None:
        try:
            expense_date = date.fromisoformat(data.expenseDate)
            update_data["expenseDate"] = expense_date.isoformat() + "T00:00:00Z"
        except ValueError:
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "Invalid expenseDate - use YYYY-MM-DD")
    ...
    updated = await prisma.expense.update(where={"id": expense_id}, data=update_data)
    logger.info(f"Expense updated: {expense_id} for production {production_id}")
    return updated
...
async def generate_expenses(production_id: str, replace: bool = False):
    ...
    # Contingency: 10% of ATL+BTL+post per industry bond/completion guarantee standard
    ...
    if contingency_amount > 0:
        ...
        logger.info(
            f"Contingency: base=${contingency_base:,.0f} x 10% = ${contingency_amount:,.0f} "
