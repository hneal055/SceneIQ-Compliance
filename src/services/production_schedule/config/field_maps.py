...
# Headerâ†’field maps for script-breakdown importers.
#
...
# Maps real-world script-breakdown CSV column headers â†’ internal Scene
...
CSV_SCENE_FIELD_MAP = {
    ...
    # Page count (decimal pages â€” 8/8 = 1.0)
    ...
    # Jurisdiction â€” stored as a raw name on Scene.jurisdiction_id by the
    # importer; the router resolves name â†’ Jurisdiction.id at persist time.
...
# Maps Movie Magic Scheduling .mms (XML) tag names â†’ internal Scene
# field names. Two sentinel values trigger specialised parsers in
# mms_importer.py:
#   "scene_heading" â†’ _parse_scene_heading()  (splits "INT. X - DAY"
#                     into loc_type / location / time_of_day)
#   "cast"          â†’ _extract_cast()         (flattens xmltodict's
...
MMS_FIELD_MAP = {
    ...
    # Scene heading â€” parsed separately by _parse_scene_heading()
    ...
    # Page count â€” converted to float by build_scene_from_mms_element()
    "PageCount"         : "page_count",
    "Pages"             : "page_count",
    "PageLength"        : "page_count",
    "Eighths"           : "page_count",

    # Cast / elements â€” split into list by _extract_cast()
    "ElementList"       : "cast",
    "Characters"        : "cast",
    "Cast"              : "cast",
    "Talent"            : "cast",

    # Jurisdiction hint (raw name â€” resolved to FK in router)
    ...
    # -------------------------------------------------------------------------
    # Intentionally NOT mapped â€” these tags are silently ignored:
