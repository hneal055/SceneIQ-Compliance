...
async def list_scenarios(current_user: TokenData = Depends(get_current_user)):
    rows = await prisma.user_scenarios.find_many(
...
async def create_scenario(
    ...
    row = await prisma.user_scenarios.create(data={
...
async def delete_scenario(
    ...
    row = await prisma.user_scenarios.find_unique(where={"id": scenario_id})
    if not row:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Scenario not found")
    if row.userId != current_user.sub:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Not your scenario")
    await prisma.user_scenarios.delete(where={"id": scenario_id})
    return None
