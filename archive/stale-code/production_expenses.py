...
# NOTE: Contingency is NOT in the templates - it is calculated dynamically
#       as 10% of (ATL + BTL + post) after all other line items are summed.
# ---------------------------------------------------------------------------
...
_TEMPLATES: dict[str, list[dict]] = {
    "feature_film": [
        ...
        # Contingency calculated dynamically - do NOT add here
    ],
    "documentary": [
...
# Categories that count toward the contingency base (ATL + BTL + post).
# insurance, legal, and other are excluded per industry bond standard.
_CONTINGENCY_BASE_CATS = {
    "labor", "equipment", "locations", "travel", "catering", "post_production", "visual_effects",
}
...
async def generate_expenses(production_id: str, replace: bool = False):
    ...
    Contingency is calculated as 10% of (ATL + BTL + post) per industry
    bond/completion guarantee standard, not as a flat percentage of total budget.
    ...
    for item in template:
        ...
        # Accumulate the contingency base: ATL + BTL + post categories only
        if item["cat"] in _CONTINGENCY_BASE_CATS:
            contingency_base += amount
    ...
    # -------------------------------------------------------------------------
    # Contingency: 10% of (ATL + BTL + post) per industry bond standard.
    # Bond companies and completion guarantors use this formula; a flat % of
    # total budget overstates contingency by including insurance/legal/other.
    # -------------------------------------------------------------------------
    contingency_amount = round(contingency_base * 0.10, 2)
    if contingency_amount > 0:
        contingency_date = date.today()
        contingency_expense = await prisma.expense.create(data={
            "productionId": production_id,
            "category":     "other",
            "description":  "Contingency Reserve (10% ATL+BTL+Post)",
            "amount":       contingency_amount,
            "expenseDate":  contingency_date.isoformat() + "T00:00:00Z",
            "isQualifying": False,
            "vendorName":   "",
        })
        created.append(contingency_expense)
        logger.info(
            f"Contingency calculated: base=${contingency_base:,.0f} x 10% = ${contingency_amount:,.0f} "
    ...
    return {
        ...
        "contingencyBase":     contingency_base,
