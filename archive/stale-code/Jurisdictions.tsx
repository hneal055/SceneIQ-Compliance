...
export default function Jurisdictions() {
  ...
  useEffect(() => {
    Promise.all([api.jurisdictions.list(), api.incentiveRules.list()])
      .then(([jurs, rls]) => {
        // Only ever keep active jurisdictions in state — inactive ones never render.
        setJurisdictions((jurs ?? []).filter((j: Jurisdiction) => j.active !== false));
        setRules(rls);
      })
      .catch(() => {})
  ...
  const filtered = useMemo(() => jurisdictions.filter(j => {
    const matchActive  = j.active !== false; // hard filter: inactive jurisdictions never show
    ...
    return matchActive && matchType && matchCountry && matchSearch;
